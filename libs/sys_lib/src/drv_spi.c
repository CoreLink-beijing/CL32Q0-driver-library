...
void spix_setup_master(spi_dev_t *spi) 
{
	if (spi == SPI0)
	{
		gpio_func_set(SPI0_CLK, FUNC0_SPI0_CLK);
		gpio_func_set(SPI0_MOSI, FUNC0_SPI0_MOSI);
		gpio_func_set(SPI0_MISO, FUNC0_SPI0_MISO);

		gpio_mode_set(SPI0_CLK, GPIO_MODE_OUT_PP);
		gpio_mode_set(SPI0_MOSI, GPIO_MODE_OUT_PULL_UP);
		gpio_mode_set(SPI0_MISO, GPIO_MODE_IN_FLOATING);

		// gpio_mode_set(SPI0_SDIO0_MOSI, GPIO_MODE_OUT_PP);
		// gpio_mode_set(SPI0_SDIO1_MISO, GPIO_MODE_IN_PULL_UP_WEAK);
	}
	else /* if (spi == SPI1) */
	{
		gpio_func_set(SPI1_CLK, FUNC1_SPI1_CLK);
		gpio_func_set(SPI1_MOSI, FUNC1_SPI1_MOSI);
		gpio_func_set(SPI1_MISO, FUNC1_SPI1_MISO);

		gpio_mode_set(SPI1_CLK, GPIO_MODE_OUT_PP);
		gpio_mode_set(SPI1_MOSI, GPIO_MODE_OUT_PULL_UP);
		gpio_mode_set(SPI1_MISO, GPIO_MODE_IN_FLOATING);

		// gpio_mode_set(SPI1_SDIO0_MOSI, GPIO_MODE_OUT_PP);
		// gpio_mode_set(SPI1_SDIO1_MISO, GPIO_MODE_IN_PULL_UP_WEAK);
	}
}
								...
								uint32_t *data_wr2, uint32_t datalen_wr2)
{
	...
	for (i=0; i<((datalen_wr1+3)>>2); i++)
	{
		data_wr1[i] = swap_endian(data_wr1[i]);
	}

	for (i=0; i<((datalen_wr2+3)>>2); i++)
	{
		data_wr2[i] = swap_endian(data_wr2[i]);
	}		
	...
	spix_setup_cmd_addr(spi, 0, 0, 0, 0);
	spix_setup_dummy(spi, 0, 0);
	spix_set_datalen(spi, datalen_wr1 * 8);
	spix_start_transaction(spi, SPI_CMD_WR, cs);
	spix_write_fifo(spi, data_wr1, datalen_wr1 * 8);
	while ((spix_get_status(spi) & 0xFFFF) != 1);
	
	spix_setup_cmd_addr(spi, 0, 0, 0, 0);
	spix_setup_dummy(spi, 0, 0);
	spix_set_datalen(spi, datalen_wr2 * 8);
	spix_start_transaction(spi, SPI_CMD_WR, cs);
	spix_write_fifo(spi, data_wr2, datalen_wr2 * 8);
	while ((spix_get_status(spi) & 0xFFFF) != 1);
